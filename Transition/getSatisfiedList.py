...
def get_user_arguments(search_term, proximity_value ,similarity_value):
    '''
    What for:
        Get the user wanted search_term, proximity_value, and similarity_value values.

    Arguments/Vars:
        search_term: The user-specified search_term.
        proximity_value: The user-specified proximity_value.
        similarity_value: The user-specified similarity_value.

    Returns:
        tuple: A tuple containing the search_term, proximity_value, and similarity_value values.
    '''
    search_term = sys.argv[1]
    proximity_value = sys.argv[2]
    similarity_value = sys.argv[3]
...
def target_list_finder(search_term,directory):
    ...
    Vars:
        ...
        target_file_list_genes_list: This is a list of strings representing the genes names from the gene list file.
        target_list: This is a set of the target_file_list_genes_list, representing the gene names from the gene list file.
    Returns:
        A tuple containing the set of genes in the gene list (target_list), the directory containing the gene lists (directory),
...
def set_target_genes_empty_dict(target_list):
    '''
    What for:
        Create an empty dictionary with the genes in 'target_list' as keys and values initialized to 0.
    Arguments:
        target_list: The set of genes to use as keys in the dictionary.
    Vars:
        target_genes_dict: A dictionary with the genes in 'target_list' as keys and values initialized to 0.
    Returns:
        target_genes_dict
    '''
...
def set_target_dict_values(target_list, directory, target_genes_dict, proximity_value):
    '''
    What for:
        Update the values in the 'target_genes_dict' dictionary for the genes that intersect with the gene lists in the 'directory'.
    Arguments:
        target_list: Set of genes in the gold gene list.
        directory: Directory containing the gene lists.
        target_genes_dict: Dictionary with the genes in 'target_list' as keys and values to be updated.
        proximity_value: proximity_value value.
    Vars:
        intersected_count: Integer that keeps track of the number of gene lists that intersect with the gold list.
    ...
    Returns:
        A tuple containing the updated 'target_genes_dict' dictionary and the count of intersecting gene lists.
    '''
    intersected_count = 0
    for filename in os.listdir(directory):
        path_list = os.path.join(directory, filename)
...
def get_approved_scientific_names_list(directory):
    '''
    What for:
        Read the names of all the files in a given directory and rewrite them in 'FiltredScientificNames_list.txt'.
    ...
    while True:
        try:
            ...
            for filename in os.listdir(directory):
                ...
                filtredScientificNames_list = open("FiltredScientificNames_list.txt", "a", encoding='utf8')
                filtredScientificNames_list.writelines(f"{scientific_name}\n")
...
def get_approved_genes_list(directory,target_genes_dict,intersected_count,similarity_value):
    ...
    Arguments:
        directory: Path to a directory.
        value: Value of the metric to be compared to the threshold.
        target_genes_dict: Dictionary that stores gene names as keys and the number of occurrences as values.
        intersected_count: Number of elements in the set being compared.
        similarity_value: Threshold value to be compared to the metric value.
    Vars:
...
def dir_checker(directory,path_target_list):
    ...
    Vars:
        path_target_genes: Represents the name of the gold list file, extracted from path_target_list.
    Returns:
        none
    '''
...
if __name__ == "__main__":
    ...
    term,proximity,similar_value = get_user_arguments(search_term='', proximity_value='', similarity_value='')
